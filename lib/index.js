...
function formatXML(str, header = false, tabs) {
  let modStr = str.slice(str.indexOf(`<${rootTag}>`) === -1 ? 0 : str.indexOf(`<${rootTag}>`) + rootTag.length + 2 , str.indexOf(`</${rootTag}>`) === -1 ? str.length : str.indexOf(`</${rootTag}>`));
  ...
  for (let i = 0; i < modStr.length + 1; i++) {
    ...
    else if (!tagValue || !tagValue.includes(`</${openingTag.slice(1, -1)}>`)) {
      tagValue += modStr[ i ];
      appendToXML = false;
    }
    if (appendToXML) {
      if (formatXML(tagValue.slice(0, (openingTag.length + 1) * -1))) {
        formattedXML += '\n' + tabs + openingTag + '\n' + formatXML(tagValue.slice(0, (openingTag.length + 1) * -1), false, tabs + '\t') + '\n' + tabs + '</' + openingTag.slice(1);
        openingTag = modStr[i];
        tagValue = '';
      } else {
  ...
  tabs = tabs.slice(-1);
  if (formattedXML.indexOf(`<${rootTag}>`) !== -1) formattedXML += '\n' + tabs + `</${rootTag}>`;
  return formattedXML;
}
