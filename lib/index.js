...
function formatXML(str, header = false, tabs) {
  let modStr = str.slice(str.indexOf('<root>') === -1 ? 0 : str.indexOf('<root>') + 6 , str.indexOf('</root>') === -1 ? str.length : str.indexOf('</root>'));
  ...
  for (let i = 0; i < modStr.length; i++) {
    if (!openingTag || openingTag.slice(-1) !== '>') {
      ...
      if (formatXML(tagValue.slice(0, openingTag.length  * -1))) {
        formattedXML += '\n' + tabs + openingTag + formatXML(tagValue.slice(0, openingTag.length * -1), false, tabs + '\t') + '\n' + tabs + '</' + openingTag.slice(1);
        openingTag = '';
        tagValue = '';
      } else {
        formattedXML += '\n' + tabs + openingTag + tagValue + '>';
  ...
  tabs = tabs.slice(-1);
  if (formattedXML.indexOf('<root>') !== -1) formattedXML += '\n' + tabs + '</root>';
  return formattedXML;
}
